...
**Output:** Salvar em `seo/01-pesquisa-demanda.md` com:
...
**Output:** `seo/02-analise-concorrencia.md` com:
...
**Output:** `seo/03-google-meu-negocio.md`
...
**Output:** `seo/04-otimizacao-on-page.md` com:
...
**Output:** `seo/05-estrategia-conteudo.md`

> Ao terminar esse passo, oferecer o encaminhamento: os temas encontrados aqui podem ir direto pro banco de pautas (`/ideias`, em `conteudo/pautas.md`) com origem "SEO — pesquisa de demanda". De lá, `/calendario` distribui no mês e `/publicar-tema` produz cada um — artigo + carrossel + legendas com um comando.
...
**Output:** `seo/06-google-ads.md` com estrutura completa, palavras-chave organizadas, copies prontas, orçamento e configurações.
...
**Output:** `seo/07-checklist-monitoramento.md`
...
**Output:** `seo/08-geo-otimizacao-ia.md` com auditoria, FAQ + schema JSON-LD, lista de ações pra aumentar citações, checklist de monitoramento.

---

## Execução

O fluxo completo é longo (8 passos, muita pesquisa real). Ao rodar `/seo` sem argumento, avisar antes de começar:
...
Cada passo é independente na leitura: sempre reler o arquivo do passo anterior em vez de assumir o que está em contexto (o fluxo pode ser retomado dias depois).
...
- Todo link de campanha sai com UTM (`utm_source`, `utm_medium`, `utm_campaign`) — sem isso não dá pra saber depois de onde veio o lead
- Antes de recomendar qualquer coisa on-page, conferir se o site tem Google Search Console e Analytics instalados. Se não tiver, isso é a ação nº 1 do plano — medir vem antes de otimizar
- Volume de busca sem ferramenta paga é sempre estimativa. Dizer isso na cara, com a lógica usada, em vez de apresentar número como fato
