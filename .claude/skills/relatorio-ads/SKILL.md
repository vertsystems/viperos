...
- **Histórico:** `campanhas/relatorios/` (criar se não existir)
...
**Cuidado com formato numérico brasileiro.** Export em pt-BR vem com vírgula decimal e ponto de milhar (`1.234,56` = mil duzentos e trinta e quatro). Se ler como padrão americano, o investimento fica 1000x errado e o relatório inteiro vira lixo. Antes de calcular:
- Detectar o separador olhando algumas linhas
- Normalizar tudo pra número antes de somar
- Conferir a sanidade do total: bate com a ordem de grandeza que o usuário espera gastar? Se o total vier absurdo (R$ 3 ou R$ 3 milhões), parar e perguntar

### Passo 2 — Comparar com a semana anterior

Buscar em `campanhas/relatorios/` o relatório anterior. Se existir, calcular variação semana vs semana:
...
Oferecer jogar essas ações no pipeline:
> "Quer que eu adicione essas ações no `tarefas.md`?"

### Passo 7 — Salvar

```
campanhas/relatorios/<YYYY-MM-DD>-relatorio.md
```
...
> "Relatório completo em `campanhas/relatorios/<data>-relatorio.md`. Quer que eu envie por email pro cliente?"
...
- **Formato numérico:** normalizar vírgula/ponto decimal do export brasileiro antes de qualquer soma.
- **Períodos comparáveis:** só comparar semana com semana (7 dias com 7 dias). Se o export tiver período diferente, avisar e normalizar por média diária.
